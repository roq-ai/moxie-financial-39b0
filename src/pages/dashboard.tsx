...
import { getStockAdvices } from 'apiSdk/stock-advices';
import { StockAdviceInterface } from 'interfaces/stock-advice';
import { getSimulatedPortfolios } from 'apiSdk/simulated-portfolios';
...
import { SimulatedPortfolioListPage as ShowCaseList } from 'pages/simulated-portfolios';

function DashboardPage() {
  const { session } = useSession();

  const stockAdviceStat = useSWR<PaginatedInterface<StockAdviceInterface>>(
    () => `/stock-advices`,
    () => getStockAdvices({ limit: 1, offset: 0 }),
  );

  const simulatedPortfolioStat = useSWR<PaginatedInterface<SimulatedPortfolioInterface>>(
    () => `/simulated-portfolios`,
    () => getSimulatedPortfolios({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Stock Advices',
      result: stockAdviceStat?.data?.totalCount,
      isLoading: stockAdviceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Simulated Portfolios',
      result: simulatedPortfolioStat?.data?.totalCount,
      isLoading: simulatedPortfolioStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Transactions',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Simulated Portfolios
                </Text>
                <NextLink href={`simulated-portfolios`} passHref legacyBehavior>
                  <Button
